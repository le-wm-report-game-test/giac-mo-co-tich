...
func test_healing_food_restores_player_to_max_health() -> void:
	...
	var item := FoodItem.new()
	item.food_type = FoodItem.FoodType.APPLE_RED
	item.heal_amount = 1.0
	world_instance.add_child(item)
	await tree.process_frame
	
	item.call("_apply_heal", player)
	
	assert_eq(health.current_health, health.max_health, "Healing food should restore player to full health")
