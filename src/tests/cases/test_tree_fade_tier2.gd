...
func test_fade_distance_boundary() -> void:
	...
	assert_almost_eq(alpha_in, 0.3, 0.01, "Should fade at 3.99m distance (just below 4.0m threshold)")
...
func test_fade_diff_x_boundary() -> void:
	...
	await tree.process_frame
	var alpha_in := get_tree_alpha(test_tree)
	assert_almost_eq(alpha_in, 0.3, 0.01, "Should fade at 2.49m diff_x (just below 2.5m threshold)")
	
	# 2. Just above 2.5m diff_x boundary: diff_x = 2.51m (diff_z = -1.0m, dist = 2.70m)
	player.global_position = Vector3(12.51, 0.0, 9.0)
	await tree.process_frame
	var alpha_out := get_tree_alpha(test_tree)
	assert_almost_eq(alpha_out, 1.0, 0.01, "Should NOT fade at 2.51m diff_x (just above 2.5m threshold)")
...
func test_fade_diff_z_boundary() -> void:
	...
	await tree.process_frame
	var alpha_in := get_tree_alpha(test_tree)
	assert_almost_eq(alpha_in, 0.3, 0.01, "Should fade at -0.01m diff_z (behind tree)")
