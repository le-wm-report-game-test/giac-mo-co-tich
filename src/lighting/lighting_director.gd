...
func _apply_quality_settings() -> void:
	...
	_sun.shadow_blur = 1.5 if cinematic else 1.0
	_sun.shadow_opacity = 0.88 if cinematic else 0.8
	...
	_environment.ssao_radius = 2.8 if cinematic else 2.0
	_environment.ssao_intensity = 1.4 if cinematic else 1.0
	_environment.ssao_power = 1.5
	_environment.ssao_detail = 0.65 if cinematic else 0.4
	_environment.ssao_horizon = 0.5
	_environment.ssil_enabled = cinematic
	_environment.ssil_radius = 4.0
